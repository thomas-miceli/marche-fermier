...
public class vendreProduitController extends VBox implements Initializable {
    ...
    public void vendreProduit() throws IOException {
        ...
        if (QuantiteVoulus.getCharacters().toString().isEmpty() || prixDeVente.getCharacters().toString().isEmpty())
            popupAlert.setText("Quantité | Prix ?");
        else if (Integer.valueOf(QuantiteVoulus.getCharacters().toString()) > participantController.getSelectedProduit().getQuantite())
            popupAlert.setText("Le participant ne possède pas assez de produit");
        else {
